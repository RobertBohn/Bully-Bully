...
$sql = "select id, stock, s1date, s1price, s2date, s2price, buydate, buyprice, amount, stop from trades where id = $_GET[d]";
$rows = mysql_query ( $sql ) or die ( 'select error: ' . mysql_error());
...
$sql = "select datediff(s.process,t.buydate) days from stocks s, trades t where t.id = $_GET[d] and t.stock = s.id";
$rows = mysql_query ( $sql ) or die ( 'select error: ' . mysql_error());
$cols = mysql_fetch_array ( $rows ) or die ( 'trade not found. id: ' . $_GET[d]); 
$days = $cols['days'];
...
addNode ( $doc, $navigation, 'pagename', $fundname . ' - Position Detail' );
