...
addNode ( $doc, $navigation, 'menu', '11' ); 
addNode ( $doc, $navigation, 'pagedescr', 'Review Stocks Held By The Fund' );
addNode ( $doc, $navigation, 'title', 'Bully Bully - Positions' );
addNode ( $doc, $navigation, 'pagename', $fundname . ' - Positions' );
...
// build content - positions

$sql = "select t.id, t.stock, s.symbol, s.company, t.amount, t.buyprice, t.buydate, datediff(s.process,t.buydate) days, (t.amount / t.buyprice) shares, s.price, t.stop, s.low120, (t.amount / t.buyprice) * (s.price - t.buyprice) profit from trades t, stocks s where t.fund = " . $_GET[t] . " and t.stock = s.id and t.selldate is null";
$rows = mysql_query ( $sql ) or die ( 'select error: ' . mysql_error());
while ( $cols = mysql_fetch_array ( $rows ) )
{    
    ...
    if ( floatval( $cols['stop'] ) > floatval( $cols['low120'] ) ) 
        addNode ( $doc, $occ, 'stop', $cols['stop'] );
    else
